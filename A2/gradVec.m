...
%% Description
% Sames as gradBatch except it outputs an array of W vectors instead of
% just one.
%% Parameters:
...
%% code:
    ...
    elseif (loss ~= 1 && loss ~= 0) % this sets the loss to zero if a number
        ...
        loss = 0;
        disp('Error: setting loss flag to 0.') % displays a warning to 
                                                 % indicate that the loss 
                                                 % indicator has been changed
    end
    wvec = zeros(N,size(X,2)); % initializes the array of Ws
    if loss
